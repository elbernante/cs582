...
from inference import make_classifier
...
def predict_images(images, classifier):
    images_plus_masks = {} 
    for f_image in images:
        img  = images[f_image]
        images_plus_masks[f_image] = classifier(img)
    return images_plus_masks
...
def write_patient_video(patient):
    ...
    cls = make_classifier("output/weights-good-120.h5")
    images       = get_patient_images(patient=patient)
    # images_masks = add_masks(images)
    images_masks = predict_images(images, cls)
