...
def split_by_label(img_ids, image_dir=TRAIN_SRC):
    ...
    imgs = get_image_labels_for_keys(img_ids, image_dir=image_dir)
    has_labels = imgs.sum(axis=tuple(range(1, len(imgs.shape)))) > 0
...
def stratified_train_val_split(labels, train=0.7, seed=None):
    ...
    print("Splitting data to train: {}, validation: {}".format(train, 1-train))
...
def process_patient(patient_id, src_dir=TRAIN_SRC, dst_dir=TRAIN_DIR):
    ...
    tqdm.write("Patient ID: {:>3}                                      Total Images: {:>4}".format(patient_id, len(imgs)))
    tqdm.write("     Before filtering -- Has Labels: {:>4}, No Labels: {:>4}, Total: {:>4}".format(len(with_labels), len(no_labels), len(with_labels) + len(no_labels)))
    no_labels = clean_up_no_labels(with_labels, no_labels,
                                   threshold=SIMILARITY_THRESHOLD,
                                   image_dir=src_dir)

    tqdm.write("     After filtering  -- Has Labels: {:>4}, No Labels: {:>4}, Total: {:>4}".format(len(with_labels), len(no_labels), len(with_labels) + len(no_labels)))
...
def filter_and_resize(src_dir=TRAIN_SRC, dst_dir=TRAIN_DIR):
    ...
    del_files(os.path.join(dst_dir, '*'))

    print("Reading raw dataset...", end='', flush=True)
...
def split_data_train_val(image_dir=TRAIN_DIR):
    ...
    print("Reading images for training...", end='', flush=True)
    ...
    #                                     seed=10)
    t_idx, v_idx = stratified_train_val_split(labels,
                                        train=(1.0 - WITHHOLD_FOR_VALIDATION), 
                                        seed=10)
    ...
    # Training set
    print("Saving training dataset...", end='', flush=True)
    ...
    t_y = t_y[..., None].astype(np.float32)

    # Save training set to file
    np.savez(TRAIN_SET_PICKLE, x=t_x, y=t_y)
    print(" Done - File saved to '{}'".format(TRAIN_SET_PICKLE), flush=True)


    # Validation set
    print("Saving validation dataset...", end='', flush=True)
    ...
    v_y = v_y[...,None].astype(np.float32)
    
    # Save validation set to file
    np.savez(VALIDATION_SET_PICKLE, x=v_x, y=v_y)
    print(" Done - File saved to '{}'".format(VALIDATION_SET_PICKLE), 
           flush=True)
    ...
    with_labels = t_y[t_y.sum(axis=tuple(range(1, len(t_y.shape)))) > 0]
    ...
    print("                     Original Mean: {}".format(t_mean))
    print("                     Centered Mean: {}".format(t_x.mean()))
    print("       Original Standard deviation: {}".format(t_std))
    print("     Normalized Standard deviation: {}".format(t_x.std()))
...
def preprocess_test_imgs(src_dir=TEST_SRC, dst_dir=TEST_DIR):
    """Resizes test images and save to numpy readable format"""
    ...
    # Resize test images
    prefix_len = len(os.path.join(src_dir, ''))
    imgs = glob.glob(os.path.join(src_dir, "*.tif"))
    for img in tqdm(imgs, desc="Resizing test images"):
        dst = os.path.join(dst_dir, img[prefix_len:])
        resize_img(img, dst)
    ...
    print("Saving test images...", end='', flush=True)
    ...
    test_x = get_images_for_keys(img_keys, image_dir=dst_dir) \
              .reshape(-1, IMAGE_HEIGHT, IMAGE_WIDTH, CHANNELS)
    test_x -= train_stats['mean']         # Center
    test_x /= train_stats['std']          # Normalize

    np.savez(TEST_SET_PICKLE, x=test_x)
    print(" Done - File saved to '{}'".format(TEST_SET_PICKLE), flush=True)
...
# Resize test images
preprocess_test_imgs(src_dir=TEST_SRC, dst_dir=TEST_DIR)
